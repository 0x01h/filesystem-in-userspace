...
translate_path (const char *path)
{
  ...
  char *rPath =
    malloc (sizeof (char) * (strlen (path) + strlen (rw_path) + 1));
...
// ls -l
static int
tidier_getattr (const char *path, struct stat *st_data)
{
...
static int
tidier_readlink (const char *path, char *buf, size_t size)
{
...
// ls
static int
tidier_readdir (const char *path, void *buf, fuse_fill_dir_t filler,
...
// touch
static int
tidier_mknod (const char *path, mode_t mode, dev_t rdev)
{
...
// mkdir
static int
tidier_mkdir (const char *path, mode_t mode)
{
...
// ln -s
static int
tidier_symlink (const char *from, const char *to)
{
...
// rename
static int
tidier_rename (const char *from, const char *to)
{
...
// ln  -l
static int
tidier_link (const char *from, const char *to)
{
...
static int
tidier_chmod (const char *path, mode_t mode)
{
...
static int
tidier_chown (const char *path, uid_t uid, gid_t gid)
{
...
static int
tidier_truncate (const char *path, off_t size)
{
...
static int
tidier_utime (const char *path, struct utimbuf *buf)
{
...
static int
tidier_open (const char *path, struct fuse_file_info *finfo)
{
...
static int
tidier_read (const char *path, char *buf, size_t size, off_t offset,
	     struct fuse_file_info *finfo)
{
  ...
  counter = 0;
  if ((strlen (selected_extension) == 4) && (selected_extension[0] == 'h')
      && (selected_extension[1] == 't') && (selected_extension[2] == 'm')
      && (selected_extension[3] == 'l'))
    {
      ...
      if (rc >= 0)
	{
	  ...
	  uint stringSize = output.size;
	  char *newBuf = (char *) malloc (stringSize);
	  rc = tidySaveString (tdoc, newBuf, &stringSize);

	  printf ("%s\n", output.bp);

	  const size_t file_size = output.size - 1;

	  if (offset >= file_size)	/* Trying to read past the end of file. */
	    return 0;

	  if (offset + stringSize > file_size)	/* Trim the read to the file size. */
	    stringSize = file_size - offset;

	  memcpy (buf, newBuf + offset, stringSize);
...
static int
tidier_write (const char *path, const char *buf, size_t size, off_t offset,
	      struct fuse_file_info *finfo)
{
...
static int
tidier_statfs (const char *path, struct statvfs *st_buf)
{
...
static int
tidier_release (const char *path, struct fuse_file_info *finfo)
{
...
static int
tidier_fsync (const char *path, int crap, struct fuse_file_info *finfo)
{
...
static int
tidier_access (const char *path, int mode)
{
 ...
 */
static int
tidier_setxattr (const char *path, const char *name, const char *value,
 ...
 */
static int
tidier_getxattr (const char *path, const char *name, char *value, size_t size)
{
 ...
 */
static int
tidier_listxattr (const char *path, char *list, size_t size)
{
 ...
 */
static int
tidier_removexattr (const char *path, const char *name)
{
...
static int
tidier_parse_opt (void *data, const char *arg, int key,
